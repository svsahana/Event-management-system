...
router.get(["/","/Attending"],function (request,response) {	  	
	if(!request.session.user){
		...
		currentuser.getAttendingEvents(function(result){
			if(result!=""){
					...
					response.render('Attending', {content:html});
			}
			else{
				response.render('Attending');
			}	
		});
...
router.get(["/logout", "/Attending"],function (request,response) {	
