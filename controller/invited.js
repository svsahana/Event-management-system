...
router.post(["/Attending","/InvitedEvents"],function (request,response){
	var event_id = request.body.id;
	var u = new U.User("", request.session.user.username, "");
	u.Attending(event_id, function(ok){
		var html = "";
...
function generateEventHtml(id, title, year, month, day, type, creator){
	...
	html += '<a id="r'+ id+'" onclick="Attending(this.id)" href="#"> <button class="invitebtn">Attending</button></a></div></div>';
