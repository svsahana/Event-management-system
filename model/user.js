...
class User{
	...
	getAttendingEvents(callback){
		var u = this.username;
		var sql = "select event_name, events.event_id, event_type, Year(event_date) as year, "+
		"Month(event_date) as month, Day(event_date) as day, users.user_name as creator" +
		" from (attending join events on attending.event_id = events.event_id) join users on users.username = events.creator" +
		" where attending.username = ? and event_date >= curdate()";
		config.con.query(sql, [u], function (err, result, fields) {
			if (err) throw err;
			if(result.length==0)
				return callback("");
	...
	Attending(id, callback){
		var u = this.username;
		var sql = "delete from event_invited where username = ? and event_id = ?";
		config.con.query(sql, [u, id], function (err, result, fields) {
			if (err) throw err;
			config.con.query("select * from attending where username = ? and event_id = ?", [u, id], function (err, result, fields) {
				if (err) throw err;
				if(result.length==0){
					sql = "insert into attending values (?, ?)"
