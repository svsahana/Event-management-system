...
class Event{
	...
	static addInvited(id, username, callback){
		config.con.query("SELECT * from event_invited where username= ? and event_id = ?", [username, id], function (err, result) {
			if (err) throw err;
				...
				config.con.query("SELECT * from attending where username= ? and event_id = ?", [username, id], function (err, result) {
					if (err) throw err;
					else if (result.length!=0){
						return callback(-1);
					}
	...
	static getDetails(id, callback){
		var sql="select user_name, users.username from attending join users on users.username = attending.username where event_id = ?";
